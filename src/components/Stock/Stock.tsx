...
const formatMarketCap = (value: number) => {
  ...
  if (numValue >= 1_000_000_000) {  // 1 billion
    return `${(numValue / 1_000_000_000).toFixed(2)} T`;
  } else if (numValue >= 1_000_000) {  // 1 million
    return `${(numValue / 1_000_000).toFixed(2)} B`;
  } else {
          ...
          <div className={styles.chart}>
            ...
            {stockResults.chartData.length === 0 && (
              <div className={styles.chartDetails}>
                <div>We&apos;re experiencing high traffic. Please try again later.</div>
              </div>
            )}
            {stockResults.chartData.length === 0 && (
