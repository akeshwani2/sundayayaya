...
import { createGmailContextMessages } from "./gmailHelpers";
...
export const getInitialMessages = (chat: Chat, data?: string): Message[] => {
  ...
  if (chat.mode === "image") {
    ...
    return [
      {
        role: "system",
        content: "You are a helpful assistant named 'Sunday'. Maintain conversation context between messages. " +
        "When asked follow-up questions, assume they refer to the previous discussion unless specified otherwise. " +
    ...
    return [
      {
        ...
        content:
          ...
          "You must must provide citations in the format of [{number}] and it starts with [{1}] and ends with [{4}].",
      },
      {
        role: "user",
  ...
  } else if (chat.mode === "gmail") {
    return createGmailContextMessages(
